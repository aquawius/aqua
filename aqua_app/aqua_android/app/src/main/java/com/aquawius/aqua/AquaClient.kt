 ...
 *
 * `hint` 是滑块下方的短说明（一句话），统一写「下限 …｜上限 …」两个数（上限 = 2/3 ×
 * 容量的结构上限），这样相邻两档能直接对比；断流余量、记忆时长等机制差异写在 `summary` 里。
 *
 * **排序约束（五条轴一起单调，"越右越抗网络"）**：容量、minTargetSlots、
 * stallPeakCapSlots、underrunPenaltySlots 从左到右单调**不减**，
 * stallPeakDecayMsPerSec 单调**不增**（记得越久）。任一出现回落，用户拖动滑块时就会遇到
 * "往右拖反而更低延迟 / 更弱保护"的自相矛盾档位（实测 LAN/50ms/公网三档曾违反此约束，
 * 2026-09-16 又修了「Wi-Fi 拥挤」与「公网」取值全同）。
 * 另有一条硬约束：**下限 ≤ 2/3 × 容量**——否则下限会被结构上限悄悄夹住、拖了不生效
 * （见 configuration_reference.md §6 的「最低水位 ↔ 缓冲容量」双向绑定）。
 */
enum class JbPreset(
    val label: String,
    val hint: String,
    ...
    ULTRA_LOW(
        label = "极低延迟",
        hint = "下限 几何地板（≥3 槽）｜上限 10 槽（约 36ms）",
        summary = "容量 16 槽 → 上限 10 槽（约 36ms），断流应对余量 4 槽（约 15ms）。\n" +
                "下限不写死，跟随几何地板（设备而定，≥3 槽 ≈ 11ms）：这是当前回放几何下压不下去的线。\n" +
    ...
    LAN_WIFI(
        label = "LAN / Wi-Fi",
        hint = "默认组合：下限 几何地板｜上限 20 槽（约 73ms）",
        summary = "core 的默认值就是按常规有线 / Wi-Fi 定标的，所以这一档全部留 0。\n" +
                "网络干净时水位会自己压到地板（≥3 槽 ≈ 11ms），抖动大时最多到 20 槽（约 73ms），\n" +
                "断流应对余量 8 槽（约 29ms）。该档位下若仍出现欠载，应优先排查 AP、网卡与发送端，\n" +
                "而非继续调整参数。",
    ),
    WIFI_SMOOTH(
        label = "Wi-Fi 稳定优先",
        hint = "推荐起点：下限 9 槽（约 33ms）｜上限 20 槽（约 73ms）",
        summary = "实测驱动的一档（软路由 / 家用 AP 的 Wi-Fi）：到达空隙中位数 20ms、\n" +
                "99 分位 29ms、偶发 40ms 以上。把水位下限锁在 9 槽（约 33ms）直接覆盖 99 分位空隙，\n" +
                "断流应对余量抬到 12 槽（约 44ms），实测可把欠载压到接近零；上限仍是 20 槽（约 73ms）。\n" +
    ...
    WIFI_CROWDED(
        label = "Wi-Fi 拥挤",
        hint = "下限 12 槽（约 44ms）｜上限 20 槽（约 73ms）",
        summary = "同一条局域网，但空口在抢：下限抬到 12 槽（约 44ms）覆盖更差的分位，\n" +
                "断流应对余量 14 槽（约 51ms）。为什么只比上一档多 2 槽、而不取公网档的 16：\n" +
                "Wi-Fi 的拥塞是瞬时的（空口重传、AP 排队），一次事故不该换来几分钟的高水位，\n" +
                "所以记忆衰减仍是 10ms/s（一次 300ms 断流约 25s 后余量开始回落）。\n" +
                "上限仍是 20 槽（约 73ms）；要更大上限、更久记忆请用右侧「公网」。",
        minTargetSlots = 12,
        stallPeakCapSlots = 14.0,
    ),
    WAN(
        label = "公网",
        hint = "下限 12 槽（约 44ms）｜上限 40 槽（约 146ms）",
        summary = "跨地域 / VPN / 4G-5G：容量 60 槽 → 上限 40 槽（约 146ms）容纳持续抖动，\n" +
                "下限同为 12 槽（约 44ms），断流应对余量 16 槽（约 58ms）覆盖常态化长间隙，\n" +
                "记忆衰减放慢到 5ms/s（同样一次 300ms 断流要约 50s 才回落，比 Wi-Fi 档久一倍）。\n" +
                "与「Wi-Fi 拥挤」的差别正在这三处：上限翻倍、余量多 2 槽、记忆久一倍。",
        jbCapacity = 60,
        minTargetSlots = 12,
    ...
    MS_50(
        label = "50 ms",
        hint = "下限 14 槽（约 51ms）｜上限 40 槽（约 146ms）",
        summary = "给水位划一条 50ms 的下限（14 槽；容量 60 槽 → 上限 40 槽 ≈ 146ms），\n" +
                "断流应对余量 16 槽（约 58ms）。适用于抖动中等且不希望延迟进一步下探的场景。\n" +
    ...
    MS_100(
        label = "100 ms",
        hint = "下限 27 槽（约 98ms）｜上限 40 槽（约 146ms）",
        summary = "在公网档之上把水位下限抬到 27 槽（约 98ms）：相比依赖欠载反馈逐次抬升，\n" +
                "直接设定下限可减少恢复过程中的欠载次数。上限 40 槽（约 146ms），断流应对余量 16 槽。\n" +
                "适用于公网且对端时钟漂移较明显的链路。",
        jbCapacity = 60,
        minTargetSlots = 27,
        stallPeakCapSlots = 16.0,
    ...
    WEAK(
        label = "弱网",
        hint = "下限 32 槽（约 117ms）｜上限 53 槽（约 193ms）",
        summary = "移动网络边缘 / 拥塞 AP：容量 80 槽（上限 53 槽 ≈ 193ms），下限 32 槽（约 117ms），\n" +
                "断流应对余量 16 槽，并把「卡顿后加固步长」翻倍到 2 槽 —— 散点丢包只把到达间隔拉到约 2 个\n" +
    ...
    MS_200(
        label = "200 ms",
        hint = "下限 55 槽（约 200ms）｜上限 80 槽（约 292ms）",
        summary = "容量 120 槽、水位下限 55 槽（约 200ms）：上限 80 槽（约 292ms）、断流应对余量 20 槽，\n" +
                "并把「卡顿后加固步长」也设为 2 槽（与「弱网」齐平：档位越靠右，散点丢包越不该\n" +
                "指望人去抖滑块）。持续劣化的链路用它换连续播放，代价为固定延迟显著增加。",
        jbCapacity = 120,
        minTargetSlots = 55,
    ...
    HOSTILE(
        label = "恶劣",
        hint = "下限 80 槽（约 292ms）｜上限 106 槽（约 387ms）",
        summary = "最后一档：容量 160 槽（上限 106 槽 ≈ 387ms），下限 80 槽，断流应对余量 24 槽、\n" +
                "记忆衰减 3ms/s、卡顿加固 3 槽。\n" +
