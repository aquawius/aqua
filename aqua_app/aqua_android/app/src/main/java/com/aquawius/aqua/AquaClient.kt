...
enum class JbPreset(
    ...
    WIFI_SMOOTH(
        label = "Wi-Fi 稳定优先",
        hint = "Wi-Fi 推荐起点：水位下限 9 槽（约 33ms），断流应对上限 12 槽",
    ...
    WIFI_CROWDED(
        label = "Wi-Fi 拥挤",
        hint = "水位下限 12 槽（约 44ms），断流应对上限 16 槽",
        summary = "比上一档再抗一层：水位下限抬到 12 槽（约 44ms）、断流应对上限 16 槽（约 58ms）。\n" +
                "适用于持续下载、AP 繁忙或同频干扰较强的环境：覆盖 30ms 量级的到达间隔突增，\n" +
                "也覆盖更长的连续断流。代价是常态延迟不低于 44ms，且断流的延迟债回落更久\n" +
                "（孤立峰值按 10ms/s 线性衰减，见 testing.md §4.2）。",
        minTargetSlots = 12,
