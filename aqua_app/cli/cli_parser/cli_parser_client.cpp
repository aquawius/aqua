    ...
    LogLevel& log_level, std::string& log_file)
{
    ...
    options.add_options()
        ...
        ("jb-min-target", "Hard lower bound in slots for the adaptive target (default 3). The effective floor is max(this, geometric floor + 1), where the geometric floor is one playback callback's packets - it always wins, so this option can only RAISE the floor and can never push the target below it. Raise it to buy a higher minimum latency floor on a link that keeps underrunning; going lower than the geometric floor is not possible through the CLI.",
