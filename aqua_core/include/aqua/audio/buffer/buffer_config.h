...
//
// 单包 transit 跳变超过本值记一次台阶事件（方向 + 幅度一并记录）。单位是**包周期**
// （与 stall 门同哲学：不同 F/采样率下毫秒数自动缩放）。2.5 包：常规 |D|（成串
// 到达 ≈1 包、单包丢失 ≈1~2 包）碰不到它；实测路径跳变（+11~+38ms ≈ 3~10 包）
// 稳稳在线外（2026-09，6715 包 5/5 检出、0 误报）。大 stall 也会同时记一笔——
// 后者另有 stall_events 行，两行对照即区分"抖动"与"路径切换"。只观测、不驱动
// 控制（ADR-1 同哲学）。固定毫秒阈值在这里是错的：F=480（10ms/包）时 10ms = 1 包，
// 单包滑动就误报——这也是全仓检索后唯一的格式相关硬伤，其余控制量天然无量纲
// （槽/包周期）或逐包换算，不受 F/采样率/声道影响。
inline constexpr double JB_TRANSIT_STEP_PACKETS = 2.5;
