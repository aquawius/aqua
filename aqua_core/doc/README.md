...
本目录描述**当前源码已经实现的系统**。阅读顺序建议：先看 `architecture.md`，再按数据流阅读 `modules/`；遇到参数、协议或线程问题，再查对应专题文档。
...
- 单位必须写清楚：`frame`=一个 sample frame（所有声道一组采样）；`slot`=一个完整 `AudioFrame`；`byte`=原始 PCM 字节。JitterBuffer 的容量单位是 slot，容量换算为 byte 时只用于内存统计。
