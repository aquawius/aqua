...
client 侧唯一的来源约束是"Audio 的 sender 必须等于 `learned_endpoint`"，而 `learned_endpoint` 来自 HeartbeatAck 的 sender，
在威胁模型内等价于"信任首个应答者"。
...
- 单包伪造 heartbeat `learned_endpoint` / 伪造 heartbeat 覆盖 server 端 endpoint：需同一 LAN 内的主动攻击者；
