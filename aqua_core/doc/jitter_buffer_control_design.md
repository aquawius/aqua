...
1. 干净链路上不为了保险付延迟：抖动项 → 0 时 target 落到下限，不是停在起始值；
2. 抖动/拥塞下不欠载：抖动项覆盖网络抖动，stall 峰值项覆盖被 stall 门剔除的尾部；
3. 任何输入都不失控：target 有结构上限、有硬下限、单次事故不买入长期延迟债务。

### 三个工作区（读懂 target 先看自己在哪个区）

实测（干净 WLAN）的大部分时间 `margin ≪ effective_min`——target 由谁定，取决于
三个量的大小关系，一次只可能是一个：

| 区 | 条件 | 谁定 target | 延迟含义 |
|---|---|---|---|
| **地板区** | `max(抖动项, stall 项) < effective_min` | 地板 + penalty（`floor_bound=1`） | 结构性 + 可闻欠载史；调 `gain`/P99 **动不了它**，只认 `--jb-min-target` / 几何地板 / penalty |
| **margin 区** | 抖动项或 stall 项超过下限 | 胜出的 margin 项（`src=` 列） | 真正的"按抖动付延迟"；只有这里调参才有效 |
| **上限区** | clamp 到 `2/3 × capacity`（`cap_bound=1`） | 结构上限 | 抖动吸收余量耗尽，再恶化靠 concealment + reanchor 续命 |
...
--jb-min-target 12 --jb-stall-peak-cap 14
```
...
Android 高级页的两档与上表对齐：**「Wi-Fi 稳定优先」= 9 槽（+ 断流应对余量 12 槽）**、
**「Wi-Fi 拥挤」= 12 槽（+ 断流应对余量 14 槽）**——前者按 p99 垫固定下限、后者再抬一档覆盖更差的分位。
（「断流应对余量」即 `--jb-stall-peak-cap`，stall 峰值项的上限；不要与容量决定的结构上限
`2/3 × 容量` 混淆——后者在 App 的 hint 里写作「上限」。）

> **Wi-Fi 档与公网档的分界不在"下限"，而在"上限 + 记忆"**：两档下限同为 12 槽（44ms），
> 但 Wi-Fi 拥挤保持容量 30（结构上限 20 槽）与 10ms/s 的记忆衰减，公网抬到容量 60
> （结构上限 40 槽）并把衰减放慢到 5ms/s。理由是事故性质不同：**空口拥塞是瞬时的**
> （重传、AP 排队，一次几十毫秒），不该换来几分钟的高水位（残留曲线见 `testing.md` §4.2）；
> **公网 / VPN / 蜂窝的断流是稀疏但反复的**，值得记住更久、并给持续抖动留更大的上限。
> 拥挤档的余量因此只比稳定优先高 2 槽，而不取公网档的 16——那是"记住更久"才配得上的余量。
