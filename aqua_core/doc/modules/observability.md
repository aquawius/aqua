...
`CMakePresets.json`：4 个 debug preset 两个都 ON，4 个 release preset 两个都 OFF。单独构建某个目标时，
各 `.cpp` 顶部有 `#ifndef ... #define ... 0` 兜底，不会因为漏传定义而编译失败。
...
- **门外（有意为之）**：
  ...
  - 启动期一次性诊断（`CLI config` / `CLI effective JB options` / `adaptive target controller` banner / `geometric floor`
    recalibrated）。理由见下一节。
...
| `JitterBuffer reanchor applied:` | `apply_reanchor()` | 请求被应用 | 时间轴跳变的实际幅度（`play_seq` 前后、清掉多少 READY 槽） |
...
| 播放头停住、声音全无 | `JitterBuffer reanchor hold-stuck fallback:`、`JitterBuffer reanchor applied:` | 看 `pulls` 是否顶到阈值、`lead` 是否卡住；`min_gap` 相关判定见 `JitterBuffer reanchor probe:` |
