...
| 日志前缀                                                          | 位置                         | 何时出现                           | 排查用途                                                                                                                                                                            |
|-------------------------------------------------------------------|------------------------------|------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `TargetController change:`                                        | `TargetController::update()` | target 变化时**每次**              | 事件驱动全量：`desired` 与 `current` 的差、`margin` 两项各自值、`strat`（`tail`/`legacy`）、`src`（`tail_p99`/`stall_peak`/`kJ`）、`floor_bind`/`cap_bind`、`penalty`、`path`、`fall_room`、`dwell_left`、`storm`、`tail_p99_ms`、`shd_leg`（影子 legacy 镜像）                   |
...
| `JitterEstimator transit step:`                                   | `observe()`                  | 单包 \|Δtransit\| ≥ `JB_TRANSIT_STEP_MS` | 路径电平跳变的方向 + 幅度 + 电平前后值。stall 行回答"到达断了多久"，这行回答"路径跳了多少"；非 stall 台阶（路由/AP 切换）只在这里出现 |
...
| `target` 在相邻两格之间反复横跳 | `TargetController change:` 的 `src` 与 `path`                                                         | 先确认是尾部摆动（`src=tail_p99`，看 `tail_p99_ms` 是否真在动）还是 `kJ` 摆动还是 `stall_peak` 摆动；kJ 看 `--jb-jitter-gain`（仅冷启动/回退路径），尾部看窗口记忆，stall 看 `--jb-stall-*`。`dwell_left` 非 0 说明锁跌正在生效，`storm=1` 说明风暴冻结中                    |
