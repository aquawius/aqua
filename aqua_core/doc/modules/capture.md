...
balance > 宽限(30ms)  真实断流：一次性合成静音补齐（空事件 / 零星小包 / 完全静默同一公式）
0 < balance ≤ 宽限    挂账结转，不合成静音（见下）
balance < 0           盈余：engine 暴发，留存抵扣未来欠账（避免迟到真实数据与已补静音重复计时）
```

宽限阈值（30ms ≈ 3 个 engine period）是必须的：唤醒相对墙钟有 ±数 ms 调度抖动，若小额欠账立即兑付，会把零样本
硬拼接进连续波形——那部分"缺失"的真实数据下一轮照常到达——拼接点波形不连续即爆音。挂账结转让抖动欠账被下一轮
