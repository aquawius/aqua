...
- 高级页顶部的"网络环境预设"直接对应 `jitter_buffer_control_design.md` §9.1 的推荐起点
  （WiFi / LAN、Wi-Fi 拥挤、公网、弱网、低延迟优先，外加"自定义"）。预设只写需要偏离
  默认的项，其余留 0 —— 这样 core 默认值演进时预设不会被冻结成一份过期的显式旧值；
  "自定义"只代表"当前组合不等于任何预设"（解锁微调），点它不改任何值；
- 选中非"自定义"预设时 JB 滑块**锁定**：预设是一组配套值，逐项改会让它失去意义；
  同时选中态是显式事实而非"反推参数等于哪个预设"，避免预设说明文字随滑块拖动
  变长变短、把整页推上推下。拖动任一滑块即转为"自定义"（预设选择按名字持久化）；
